...
public class SmsService
{
    // Updated connection string
     private readonly string _connectionString = "Data Source=db13167.public.databaseasp.net;Initial Catalog=db13167;Integrated Security=False;Persist Security Info=False;User ID=db13167;Password=8Bw@Tn#52?Cj;TrustServerCertificate=True;Encrypt=False; MultipleActiveResultSets=True;";
    ...
    public async Task InsertSmsAsync(SmsMessage smsMessage)
    {
        using (var connection = new SqlConnection(_connectionString)) // SqlConnection
        {
            ...
            var checkQuery = @"
                SELECT COUNT(1)
                FROM SmsRow
                WHERE Sender = @Sender AND SmsDateTime = @SmsDateTime AND Pan=@Pan";
            ...
            {
                var insertQuery = @"
                    INSERT INTO SmsRow (SmsRowId,Sender, SmsBody, SmsDateTime,Pan,EmailId)
                    VALUES (NEWID(),@Sender, @SmsBody, @SmsDateTime,@Pan,@EmailId)";
